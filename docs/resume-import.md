...
- PDF: deve iniciar com `%PDF-`, nao pode estar protegido por senha, precisa
  ter paginas e no maximo 30 paginas. PDF sem texto extraivel retorna a mensagem
  "Não foi possível encontrar texto suficiente neste PDF." O sistema nao faz
  OCR.
...
sanitizado, hash do conteudo, contadores e status. Cada `ResumeImportCandidate`
guarda tipo, decisao, carga estruturada revisavel, confianca, explicacao e
trecho curto de origem. O arquivo bruto, bytes originais, caminho temporario e
texto completo extraido nao sao persistidos.
...
O parser nao inventa informacoes ausentes. Linhas de contato sao ignoradas.
